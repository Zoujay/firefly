...
public class JobBuildServiceImpl implements IJobBuildService {
  ...
  public Boolean updateJobBuildStatus(
    ...
    int result =
        jobBuildDao.transitionJobBuildStatus(jobBuildID, expectedStatus, status, executionAttempt);
    return result == 1;
  }
  ...
  public List<JobBuildDto> getTailJobBuildsByStageBuildID(Long stageConfigID, Long stageBuildID) {
    return getTailJobBuilds(stageConfigID, jobBuildDao.getJobBuildsByStageBuildID(stageBuildID));
  }

  /**
   * Uses a locking read after the Stage row has been locked. Locking reads bypass a stale
   * REPEATABLE READ snapshot, so the last terminal Job transaction observes every previously
  ...
  private JobBuild assembleJobBuild(JobBuildDto jobBuildDto, BuildStatus status) {
    ...
    jobBuild
        .setStageBuildID(jobBuildDto.getStageBuildID())
