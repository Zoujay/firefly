...
public class MessageCenter {
  ...
  public Boolean onStageMessage(TriggerStageMessage stageMessage) {
    ...
    Boolean updated =
        stageBuildService.updateStageBuildStatusByID(buildStatus, stageBuildID, executionAttempt);
    if (!Boolean.TRUE.equals(updated)) {
      throw new IllegalStateException(
          "Failed to update stage build: stageBuildID=" + stageBuildID + ", status=" + buildStatus);
    }
    ...
    if (buildStatus.equals(BuildStatus.SUCCESS)) {
      ...
      if (nextStageBuild == null) {
        ...
        triggerPipelineMessage
            ...
            .setMessageUUID(
                BusinessMessageUUID.pipeline(pipelineBuildID, executionAttempt, buildStatus))
    ...
    if (buildStatus.equals(BuildStatus.FAILURE)) {
      ...
      triggerPipelineMessage
          .setPipelineBuildID(pipelineBuildID)
          .setMessageUUID(
              BusinessMessageUUID.pipeline(pipelineBuildID, executionAttempt, BuildStatus.FAILURE))
  ...
  public Boolean onJobMessage(TriggerJobMessage jobMessage) {
    ...
    if (!Boolean.TRUE.equals(result)) {
      throw new IllegalStateException(
          "Failed to update job build: jobBuildID=" + jobBuildID + ", status=" + buildStatus);
    }
    TriggerStageMessage triggerStageMessage =
        this.assembleTriggerStageByJobMessage(stageBuildID, BuildStatus.RUNNING, executionAttempt);
    if (buildStatus == BuildStatus.SUCCESS) {
      ...
      JobBuildDto nextJobBuildDto =
          findNextRunnableJob(stageConfigDto.getId(), stageBuildID, jobBuildDto.getJobConfigID());
      if (nextJobBuildDto != null) {
        ...
        triggerJobMessage.setMessageUUID(
            BusinessMessageUUID.job(
                nextJobBuildID, nextJobBuildDto.getExecutionAttempt(), BuildStatus.RUNNING));
    ...
    if (buildStatus.equals(BuildStatus.FAILURE)) {
      Boolean transitioned =
          stageBuildService.transitionStageBuildStatus(
              stageBuildID, BuildStatus.RUNNING, BuildStatus.FAILURE, executionAttempt);
      if (!Boolean.TRUE.equals(transitioned)) {
  ...
  public Boolean onPluginMessage(TriggerPluginMessage pluginMessage) {
    ...
    if (!isTerminalStatus(status)) {
      throw new IllegalStateException(
          "Plugin message is not terminal: pluginBuildID=" + pluginBuildID + ", status=" + status);
    }
    ...
    if (!Boolean.TRUE.equals(pluginResult)) {
      throw new IllegalStateException(
          "Failed to update plugin build: pluginBuildID=" + pluginBuildID + ", status=" + status);
    }
  ...
  private List<TriggerJobMessage> assembleTriggerRunnableJobMessages(
    ...
    for (JobBuildDto jobBuildDto : jobBuildDtos) {
      ...
      jobMessage
          .setMessageUUID(
              BusinessMessageUUID.job(jobBuildID, executionAttempt, BuildStatus.RUNNING))
          .setJobBuildID(jobBuildID)
          .setExecutionAttempt(executionAttempt)
  ...
  private JobBuildDto findNextRunnableJob(
    ...
    while (relation != null && relation.getNextJobID() != null && relation.getNextJobID() > 0L) {
      ...
      JobBuildDto candidate =
          jobBuildService.getJobBuildByJobConfigIDAndStageBuildID(nextJobConfigID, stageBuildID);
      if (candidate == null) {
        return null;
      }
      if (candidate.getStatus() != BuildStatus.SUCCESS) {
  ...
  private void completeStageIfReady(
    ...
    TriggerStageMessage stageMessage =
        assembleTriggerStageByJobMessage(stageBuildID, BuildStatus.SUCCESS, executionAttempt);
