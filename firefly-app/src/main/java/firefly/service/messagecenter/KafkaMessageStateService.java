...
public class KafkaMessageStateService {
     ...
     *
     * <p>The claim is committed before business processing starts so an
     * application crash remains visible as PROCESSING. Because this is a
     * conditional database update, concurrent automatic or manual attempts for
     * the same UUID cannot both execute the business handler.</p>
     */
    ...
    public boolean claim(
        ...
        int updated = dao(category).claimForProcessing(
                messageUUID,
                List.of(
     ...
     *
     * <p>MANDATORY is intentional: the success marker must never commit in a
     * standalone transaction. It must commit or roll back together with the
     * Pipeline/Stage/Job/Plugin changes and the downstream Outbox insert.</p>
     */
    ...
    public boolean markSuccess(
        ...
        int updated = dao(category).markProcessingSuccess(
                messageUUID,
                MessageProcessingStatus.PROCESSING,
    ...
    /**
     * Records PROCESSING -> FAILURE after the business transaction has rolled
     * back. A new transaction is required so the failure evidence survives.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public boolean markFailure(
        ...
        int updated = dao(category).markProcessingFailure(
                messageUUID,
                MessageProcessingStatus.PROCESSING,
                MessageProcessingStatus.FAILURE,
                processorID,
    ...
    public boolean resetProcessing(
        ...
        String error = StringUtils.left(
                StringUtils.defaultIfBlank(reason, "MANUAL_RESET"),
                2048
        );
        int updated = dao(category).markProcessingFailure(
                messageUUID,
    ...
    public KafkaMessage getRequired(
            MessageCategory category,
            String messageUUID
    ) {
        return dao(category).findByMessageUUID(messageUUID)
    ...
    public KafkaMessageProcessingResponse getResponse(
        ...
        return KafkaMessageProcessingResponse.from(
                category,
                getRequired(category, messageUUID)
        );
    }
    ...
    public Page<KafkaMessageProcessingResponse> getResponses(
        ...
        return dao(category)
                ...
                .map(message -> KafkaMessageProcessingResponse.from(
                        category,
                        message
                ));
    }

    private IKafkaMessageDao<? extends KafkaMessage> dao(
            MessageCategory category
    ) {
