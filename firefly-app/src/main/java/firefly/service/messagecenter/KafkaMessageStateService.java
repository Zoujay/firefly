...
public class KafkaMessageStateService {
   ...
   *
   * <p>MANDATORY is intentional: the success marker must never commit in a standalone transaction.
   * It must commit or roll back together with the Pipeline/Stage/Job/Plugin changes and the
   * downstream Outbox insert.
   */
  ...
  /**
   * Records PROCESSING -> FAILURE after the business transaction has rolled back. A new transaction
   * is required so the failure evidence survives.
   */
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  ...
  public boolean resetProcessing(
      MessageCategory category, String messageUUID, String expectedProcessorID, String reason) {
  ...
  @Transactional(readOnly = true)
  public KafkaMessageProcessingResponse getResponse(MessageCategory category, String messageUUID) {
