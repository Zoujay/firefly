 ...
 *
 * <p>The business state, every downstream Outbox insert, and the Inbox
 * PROCESSING -> SUCCESS transition commit together. Any exception rolls all of
 * them back; the coordinator then records FAILURE in a separate transaction.</p>
 */
@Service
public class KafkaMessageProcessingTransaction {
    ...
    public void process(
        ...
        KafkaMessage archivedMessage =
                stateService.getRequired(category, messageUUID);
        if (archivedMessage.getProcessingStatus()
                != MessageProcessingStatus.PROCESSING
                || !StringUtils.equals(
                archivedMessage.getProcessorID(),
                processorID
        )) {
            throw new IllegalStateException(
                    "Kafka message is not owned by this processor: "
                            + messageUUID
            );
        }
        ...
        if (!stateService.markSuccess(
                category,
                messageUUID,
                processorID
        )) {
            throw new IllegalStateException(
                    "Failed to transition Inbox message to SUCCESS: "
                            + messageUUID
            );
        }
    }
    ...
    private void dispatch(
        ...
        switch (category) {
            case PIPELINE -> messageCenter.onPipelineMessage(
                    objectMapper.readValue(
                            payload,
                            TriggerPipelineMessage.class
                    )
            );
            case STAGE -> messageCenter.onStageMessage(
                    objectMapper.readValue(
                            payload,
                            TriggerStageMessage.class
                    )
            );
            case JOB -> messageCenter.onJobMessage(
                    objectMapper.readValue(
                            payload,
                            TriggerJobMessage.class
                    )
            );
            case PLUGIN -> messageCenter.onPluginMessage(
                    objectMapper.readValue(
                            payload,
