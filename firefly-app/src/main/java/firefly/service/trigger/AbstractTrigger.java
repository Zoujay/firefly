...
public abstract class AbstractTrigger<T extends BaseTriggerEntity, M extends BaseMessage>
  ...
  public void dispatch(BaseMessage baseMessage) {
    ...
    if (triggerEntity == null || triggerEntity.getId() == null || triggerEntity.getId() <= 0) {
      throw new IllegalStateException("Trigger record ID was not generated: " + getTriggerOrigin());
    }
     ...
     */
    outboxService.enqueue(KafkaConfiguration.PIPELINE_TOPIC, pipelineMessage);
  }

  private M validateAndCast(BaseMessage message) {
    if (message == null) {
      throw new IllegalArgumentException("Trigger message must not be null");
    }
    if (message.getTriggerOrigin() == null) {
      throw new IllegalArgumentException("Trigger origin must not be null");
    }
  ...
  private void validateRequiredFields(M message) {
    if (message.getPipelineID() == null || message.getPipelineID() <= 0) {
      throw new IllegalArgumentException("pipelineID must be positive");
    }
    if (message.getPipelineBuildID() == null || message.getPipelineBuildID() <= 0) {
      throw new IllegalArgumentException("pipelineBuildID must be positive");
    }
    if (message.getExecutionAttempt() == null || message.getExecutionAttempt() < 0) {
