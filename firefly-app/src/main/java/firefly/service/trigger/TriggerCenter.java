...
public class TriggerCenter implements ITriggerCenter {
    ...
    private final Map<TriggerOrigin, ITrigger<? extends BaseMessage>> triggerMap;
    ...
    public void dispatch(BaseMessage message) {
        if (message == null || message.getTriggerOrigin() == null) {
            throw new IllegalArgumentException("Trigger message and origin must not be null");
        }

        TriggerOrigin triggerOrigin = message.getTriggerOrigin();
        ITrigger<? extends BaseMessage> trigger = triggerMap.get(triggerOrigin);
        if (trigger == null) {
            throw new IllegalStateException("Unsupported Trigger origin: " + triggerOrigin);
        }
