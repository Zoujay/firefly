...
public class OutboxStateService {
    ...
    /**
     * Atomically performs PENDING/FAILED -> PUBLISHING in an independent
     * transaction. Only the caller whose conditional update affects one row
     * may send the event, preventing concurrent manual and after-commit sends.
     */
    ...
    public Optional<OutboxPublishTask> claim(
        ...
        return Optional.of(new OutboxPublishTask(
                event.getId(),
                event.getTopic(),
                event.getMessageKey(),
                event.getPayload(),
    ...
    /**
     * Completes the normal PUBLISHING -> SENT transition. Matching the
    ...
    /**
     * Persists PUBLISHING -> FAILED after Kafka reports an error. FAILED is a
     * terminal automatic state here; an operator explicitly starts any retry.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    ...
    /**
     * Converts an abandoned PUBLISHING row to FAILED after an operator has
     * confirmed that its publisher is no longer active.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    ...
    @Transactional(readOnly = true)
    public Page<OutboxEventResponse> getResponses(
            OutboxStatus status,
            Pageable pageable
    ) {
