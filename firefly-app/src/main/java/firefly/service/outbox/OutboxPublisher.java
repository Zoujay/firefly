...
public class OutboxPublisher {
  ...
  /**
   * Performs exactly one publication attempt; there is deliberately no loop, scheduler, or database
   * poller. Normal writes call this once after commit, while PENDING/FAILED records are retried
   * through the manual administration endpoint.
   *
   * <p>State transitions are PENDING/FAILED -> PUBLISHING -> SENT or FAILED. A process crash while
   * sending leaves PUBLISHING, which an operator first resets to FAILED after checking Kafka, then
   * explicitly retries.
   */
  public boolean publishOnce(Long outboxID) {
    ...
    try {
      ...
      log.error(
          "Failed to publish Outbox event {}; manual retry is required", task.id(), exception);
      return false;
    }
  }
}
