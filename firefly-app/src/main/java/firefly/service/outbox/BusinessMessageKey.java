 ...
 * message. The key deliberately excludes status and message UUID: events for
 * the same build attempt stay in one partition and retain their order, while
 * unrelated builds can still be consumed in parallel.
 */
public final class BusinessMessageKey {
    ...
    public static String from(KafkaBusinessMessage message) {
        if (message instanceof TriggerPipelineMessage pipeline) {
            return key(
                    "pipeline",
                    pipeline.getPipelineBuildID(),
                    pipeline.getExecutionAttempt()
            );
        }
        if (message instanceof TriggerStageMessage stage) {
            return key(
                    "stage",
                    stage.getStageBuildID(),
                    stage.getExecutionAttempt()
            );
        }
