...
public interface IJobBuildDao extends JpaRepository<JobBuild, Long> {
  ...
  List<JobBuild> getJobBuildsByStageBuildIDForUpdate(@Param("stageBuildID") Long stageBuildID);
  ...
  @Query(
      "select j from JobBuild as j where j.jobID = ?1 and j.stageBuildID = ?2 order by j.id desc"
          + " limit 1")
