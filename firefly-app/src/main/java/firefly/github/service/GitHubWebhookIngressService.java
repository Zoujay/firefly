...
public class GitHubWebhookIngressService {
    ...
    try {
      GitHubDeliveryWriteResult result =
          deliveryWriter.persist(
              subscription, event, new String(rawPayload, StandardCharsets.UTF_8), hookId);
      if (result.rejected()) {
        throw forbidden();
      }
  ...
  private GitHubRepositorySubscriptionEntity locate(String eventType, Long hookId, Long targetId) {
