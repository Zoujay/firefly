...
class KafkaMessageJpaIntegrationTests {
  ...
  void savesTheSameBusinessUUIDOnlyOnceAcrossDifferentKafkaOffsets() {
    ...
    KafkaMessageSaveResult firstResult =
        kafkaMessageStore.savePipelineMessages(
            List.of(new ConsumerRecord<>("pipeline_message", 0, 9_999_991L, messageUUID, payload)));
    KafkaMessageSaveResult duplicateResult =
        kafkaMessageStore.savePipelineMessages(
            List.of(new ConsumerRecord<>("pipeline_message", 0, 9_999_992L, messageUUID, payload)));
  ...
  void onlyOneConcurrentInsertCanClaimTheSameBusinessUUID() throws Exception {
    ...
    try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
      var firstTask = executor.submit(() -> kafkaMessageStore.savePipelineMessages(List.of(first)));
      var secondTask =
