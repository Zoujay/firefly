...
class MessageListenerTests {
  ...
  void extractUUIDFromPayload() throws Exception {
    lenient()
        ...
        .thenAnswer(
            invocation ->
                objectMapper
                    .readTree(((ConsumerRecord<?, ?>) invocation.getArgument(0)).value().toString())
                    .get("messageUUID")
                    .asText());
  }
  ...
  void persistsAcknowledgesAndDispatchesEveryPipelineMessage() throws Exception {
    ...
    List<ConsumerRecord<String, String>> messages =
        List.of(
            record("pipeline_message", 0, 10L, first), record("pipeline_message", 0, 11L, second));
  ...
  void dispatchesBusinessProcessingOnVirtualThread() throws Exception {
    ...
    when(kafkaMessageStore.savePipelineMessages(messages)).thenReturn(saved(messages));
    when(processingCoordinator.process(MessageCategory.PIPELINE, pipelineMessage.getMessageUUID()))
        .thenAnswer(
            invocation -> {
  ...
  void keepsTheBatchAcknowledgedWhenAnArchivedPayloadCannotBeParsed() {
    ...
    List<ConsumerRecord<String, String>> messages =
        List.of(new ConsumerRecord<>("pipeline_message", 0, 10L, messageUUID, "{not-valid-json"));
    when(kafkaMessageStore.savePipelineMessages(messages)).thenReturn(saved(messages));
