...
public class GitHubApiClient {
    ...
    public List<GitHubRepository> listRepositories(String token) {
        requireToken(token);
        try {
            List<GitHubRepository> repositories = authenticate(
                    restClient.get().uri(properties.getApiBaseUrl().resolve(
    ...
    public GitHubRepository getRepository(String token, String owner, String repository) {
        ...
        try {
            GitHubRepository result = authenticate(
                    restClient.get().uri(repositoryUri(owner, repository, "")),
                    token
            )
                    .retrieve()
    ...
    public List<GitHubWebhook> listWebhooks(String token, String owner, String repository) {
        validateRepository(owner, repository);
        try {
            List<GitHubWebhook> hooks = authenticate(
    ...
    public GitHubWebhook createWebhook(
        ...
        try {
            GitHubWebhook hook = authenticate(
                    restClient.post()
    ...
    public void pingWebhook(
        ...
        try {
            authenticate(
                    restClient.post().uri(repositoryUri(
                            owner,
                            repository,
    ...
    public void deleteWebhook(
        ...
        try {
            authenticate(
                    restClient.delete().uri(repositoryUri(
                            owner,
                            repository,
                            "/hooks/" + webhookId
                    )),
    ...
    private URI repositoryUri(String owner, String repository, String suffix) {
        return properties.getApiBaseUrl().resolve(
                "/repos/" + owner + "/" + repository + suffix
        );
    }
